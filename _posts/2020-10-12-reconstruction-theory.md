...
The exponential Dinur-Nissim attack is quite powerful, as it recovers 80% of the secret bits even from answers with 5% error, but it has the drawback that it requires asking \\\(2^\dsize\\\) queries to a dataset with \\\(\dsize\\\) users.  Note that this is inherent to some extent.  Suppose we randomly subsample 60% of the dataset and answer the queries using only this subset by rescaling appropriately.  Clearly no attacker can reconstruct 80% of the secret bits, since some of them are effectively deleted.  However, the guarantees of random sampling tell us that any set of \\\(\qsize\\\) queries will be answered with maximum error \\\( \acc n =  O(\sqrt{n \log \qsize})\\\), so we can answer \\\( 2^{\Omega(n)} \\\) queries with \\\(5%\\\) error while provably preventing this sort of reconstruction.[^1]
...
* The reconstruction attacks still apply to the seemingly more general data model in which the private 
	...
	we can take \\\(\uni = \{\pbs_1, \ldots, \pbs_\dsize\}\\\), and we can interpret the secret bits \\\(\sb_i\\\) to
	indicate whether \\\(\pbs_i\\\) is an element of \\\(\ds\\\). Then the reconstruction attacks allow us to determine, 
	up to some error, which elements of \\\(\uni\\\) are contained in \\\(\ds\\\).  In the setting, the attack is sometimes
	called *membership inference*.  
...
[^1]: That isn't to say we should think of the mechanism that randomly subsamples 60% of the users' is private, just that it prevents complete reconstruction of the dataset.}