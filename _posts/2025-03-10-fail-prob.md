...
Thus the wiggle room we're asking for is comparable to (or better than) what we'd get from the exponential mechanism, assuming \\\(\\log
\(1/\\delta\) \\le O\(\\log\|\\mathcal{Y}\|\)\\\).

Now we can modify the algorithm \\\(M : \\mathcal{X}^n \\to \\mathcal{Y}\\\) to also output an estimate of the loss using truncated Laplace noise. Call this new algorithm \\\(\\overline{M} : \\mathcal{X}^n \\to \\mathcal{Y} \\times \\mathbb{R}\\\). If \\\(M\\\) is \\\(\(\\varepsilon,\\delta\)\\\)-DP, we can make \\\(\\overline{M}\\\) satisfy \\\(\(2\\varepsilon,2\\delta\)\\\)-DP and guaratee that the error of the loss estimate is \\\(\\le \\tau = O\(\\log\(1/\\delta\)/\\varepsilon\)\\\) with probability 1.[^4]
...
To recap, we have a \\\(\(2\\varepsilon,2\\delta\)\\\)-DP algorithm \\\(\\overline{M} : \\mathcal{X}^n \\to \\mathcal{Y} \\times \\mathbb{R}\\\) with the following properties. Let \\\(x \\in \\mathcal{X}^n\\\) be arbitrary. Then, for \\\( \(Y,Z\) \\gets \\overline{M}\(x\)\\\), we have
...
> Assume \\\(\\overline{M} : \\mathcal{X}^n \\to \\mathcal{Y} \\times \\mathbb{R}\\\) is \\\(\(\\overline\\varepsilon,\\overline\\delta\)\\\)-differentially private and, for all inputs \\\(x\\in\\mathcal{X}^n\\\), if \\\(\(Y,Z\)\\gets\\overline{M}\(x\)\\\), then \\\(\\mathbb{P}\[Z\\le\\alpha+\\tau\]\\ge1-\\beta&gt;0\\\).
...
As long as \\\(1-\\beta \\ge \\Omega\(1\)\\\), [Theorem 1](#thm1) gives \\\(\\widetilde{\\varepsilon} = 2\\overline{\\varepsilon} + O\(\\overline{\\delta}\)\\\) and \\\(\\widetilde{\\delta}=O\(\\overline{\\delta}\)\\\).  

We were given a \\\(\(\\varepsilon,\\delta\)\\\)-DP algorithm \\\(M\\\) and we constructed \\\(\\overline{M}\\\) such that it is \\\(\(\\overline\\varepsilon,\\overline\\delta\)\\\)-DP with \\\(\\overline{\\varepsilon}=2\\varepsilon\\\) and \\\(\\overline{\\delta}=2\\delta\\\). Thus [Theorem 1](#thm1) tells us that \\\(\\widetilde{M}\\\) is \\\(\(4\\varepsilon+O\(\\delta\),O\(\\delta\)\)\\\)-DP.
...
What if, instead of eliminating the failure probability, we just want to reduce it? 
...
This method is particularly useful in settings where the initial success probability is low, such as when there is some element of random guessing involved.  
...
In particular, if we obtain bounds on the expected error, then we can obtain high-probability bounds via this method.
