...
We started with a \\\(\(\\varepsilon,\\delta\)\\\)-DP algorithm \\\(M\\\) and we constructed \\\(\\overline{M}\\\) such that it is \\\(\(\\overline\\varepsilon,\\overline\\delta\)\\\)-DP with \\\(\\overline{\\varepsilon}=2\\varepsilon\\\) and \\\(\\overline{\\delta}=2\\delta\\\). Thus [Theorem 1](#thm1) tells us that \\\(\\widetilde{M}\\\) is \\\(\(4\\varepsilon+O\(\\delta\),O\(\\delta\)\)\\\)-DP. In terms of utility, we went from \\\(\\forall x ~ \\mathbb{P}\[\\ell\(M\(x\),x\) \\le \\alpha \] \ge 1-\\beta \\\) to \\\(\\forall x ~ \\mathbb{P}\[\\ell\(\\widetilde{M}\(x\),x\) \\le \\alpha + 2\\tau \] = 1\\\).

Note that our utility failure probability \\\(\\beta\\\) appears in the privacy parameters of [Theorem 1](#thm1).[^fail] This is a bit unintuitive, but we saw how it can happen earlier with the trick of absorbing the utility failure as a privacy failure. The dependence here is milder than before; e.g., we can start with a high utility failure probability, e.g. \\\(\\beta=0.5\\\), and still get a low final privacy failure probability \\\(\\widetilde{\\delta}\\le10^{-6}\\\).
